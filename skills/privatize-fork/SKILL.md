...
- **启用了翻译模块**：
  ...
  2. 读 `references/translate-docs.md`，**按其流程内联执行翻译**：其中**「判断待翻清单」一步改调脚本**（这步要读每篇译文 frontmatter 再跑 git diff 比对，模型最易偷懒）——从 `CONVENTIONS.md` 白名单读出源文件后 `bash "$SKILL_DIR/scripts/translate-plan.sh" <白名单文件...>`，据其输出的 `TRANSLATE`/`RETRANSLATE` 再翻译（CC 用 `Task`、Codex 用其 subagent 等各自的并发原语并行派发，仅在确无并行能力时才顺序做）。**增量更新**——脚本标 `SKIP` 的译文跳过。
