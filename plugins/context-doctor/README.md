...
**会话快照（默认开启）**：脚本读不到 Host 的上下文窗口，故由「跑技能的模型」自报当前会话可见的工具/技能写成精简 JSON、带 `--session-snapshot` 跑——报告即标出每项是否 `visible_in_session`。**仅对宿主平台有意义**（在 Claude 里跑只标 Claude 那侧，Codex 侧为「—」未对照；反之亦然）。UUID 命名空间的连接器（如 claude.ai HyperFrames）可在快照里加 `source_hint` 兜底匹配。
